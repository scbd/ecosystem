...
const browserEmsPackages = {
  '\'@scbd/locale'               : '\'https://cdn.cbd.int/@scbd/locale',
  '\'@scbd/default-options'      : '\'https://cdn.cbd.int/@scbd/default-options',
  '\'@scbd/am4-map-base'         : '\'https://cdn.cbd.int/@scbd/am4-map-base',
  '\'@amcharts/amcharts4'        : '\'https://cdn.cbd.int/@amcharts/amcharts4',
  '\'@amcharts/amcharts4-geodata': '\'https://cdn.cbd.int/@amcharts/amcharts4-geodata'
}
...
module.exports = { vueUse, hasWindowSupport, NAME, PASCAL_PKG_NAME, PKG_NAME, banner, external, isComponent, fileNamePreFix, cleanCss, fixSourceMapsDev, useCdn  }

const vDataRegex  = /(?<=\[data-v-)(.{8})(?=])/
const base        = path.resolve(__dirname, '../dist')

async function useCdn (dir, absBase=base+'/browser'){
  const dirFd = dir || await fs.promises.opendir(base+'/browser')

  for await (const dirent of dirFd){
    const fileName = path.resolve(absBase, dirent.name)
    
    if(dirent.isDirectory()) useCdn(await fs.promises.opendir(fileName), fileName)
    ...
    if(dirent.isFile()){
      ...
      for (const search in browserEmsPackages)
        replaceInFile(fileName, search, browserEmsPackages[search])
    }
  }
}

async function fixSourceMapsDev (dir, absBase=base){
  const dirFd = dir || await fs.promises.opendir(base)

  for await (const dirent of dirFd){
    const fileName = path.resolve(absBase, dirent.name)

    if(dirent.isDirectory()) fixSourceMapsDev(await fs.promises.opendir(fileName), fileName)
